...
export default function StepTemplate({
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 pt-8 pb-6 sm:pt-10 sm:pb-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 animate-fade-up" style={{ animationDelay: "0.1s" }}>
        {COACHABLE_TEMPLATES.map((template, i) => {
          ...
          return (
            <button
              ...
              className={`
                group relative flex items-start gap-3.5 p-4 rounded-lg
        ...
        >
          ...
          >
            <PencilIcon className={`w-5 h-5 ${customMode ? "text-brand-700" : "text-text-tertiary group-hover:text-brand-500"}`} />
          </div>
          <div className="min-w-0 pt-0.5 pr-6">
            <p
              className={`text-sm font-semibold transition-colors duration-200 ${
                customMode ? "text-text-primary" : "text-text-primary"
              }`}
            >
              Something else
            </p>
