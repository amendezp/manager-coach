...
export default function StepTemplateContext({
  ...
  // Fetch insights when a team member is selected
  const [memberInsights, setMemberInsights] = useState<string | null>(null);
  const [insightsLoading, setInsightsLoading] = useState(false);

  useEffect(() => {
    if (!context.teamMemberId || isGuest) {
      setMemberInsights(null);
      return;
    }
    setInsightsLoading(true);
    fetch(`/api/team-members/${context.teamMemberId}/insights`, { method: "POST" })
      .then((r) => (r.ok ? r.json() : { insights: null }))
      .then((d) => setMemberInsights(d.insights || null))
      .catch(() => setMemberInsights(null))
      .finally(() => setInsightsLoading(false));
  }, [context.teamMemberId, isGuest]);
              ...
              {insightsLoading && (
                <div className="p-3 rounded-lg border border-brand-200 bg-brand-50/50">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-text-tertiary">Loading insights...</span>
                  </div>
                </div>
              )}
              {memberInsights && !insightsLoading && (
                <div className="p-3 rounded-lg border border-brand-200 bg-brand-50/30">
                  <div className="flex items-center gap-1.5 mb-2">
                    ...
                    <p className="text-[11px] font-semibold text-text-primary">Insights from past sessions</p>
                  </div>
                  <div className="text-[11px] text-text-secondary leading-relaxed whitespace-pre-wrap max-h-48 overflow-y-auto">
                    {memberInsights}
                  </div>
                </div>
              )}
