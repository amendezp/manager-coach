...
import { CalendarIcon, getIcon } from "@/components/Icons";
import { COPILOT_SCENARIOS } from "@/lib/scenarios";
    ...
    <div className="max-w-2xl mx-auto px-4 pt-8 pb-6 sm:pt-14 sm:pb-8">
      <div className="text-center mb-10 animate-fade-up">
        ...
        <p className="text-text-secondary text-sm sm:text-base leading-relaxed max-w-lg mx-auto">
          Prepare structured agendas for your 1:1s using the OSKAR framework,
          and turn post-meeting notes into polished feedback.
        </p>
      </div>
      ...
      <div className="mb-8 animate-fade-up" style={{ animationDelay: "0.1s" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {COPILOT_SCENARIOS.map((scenario, i) => {
            ...
            return (
              <button
                ...
                `}
              >
                <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-brand-50 border border-brand-100/80 flex items-center justify-center group-hover:bg-brand-100 group-hover:border-brand-200 transition-colors duration-200">
                  <Icon className="w-5 h-5 text-brand-600" />
                </div>
                <div className="min-w-0 pt-0.5">
                  <p className="text-sm font-semibold text-text-primary group-hover:text-brand-700 transition-colors duration-200">
                    {scenario.title}
                  </p>
      ...
      <div className="text-center animate-fade-in" style={{ animationDelay: "0.3s" }}>
        <p className="text-xs text-text-tertiary">
          Or describe your upcoming 1:1 below &mdash; your conversations are
...
export default function CopilotPage() {
  return (
    <ChatInterface
      flow="copilot"
      title="1:1 Co-Pilot"
      subtitle="Meeting prep & feedback synthesis"
      icon={<CalendarIcon className="w-[18px] h-[18px] text-white" />}
      placeholder="Describe your upcoming 1:1 or paste post-meeting notes..."
